...
class PostIdea extends StatefulWidget {
  ...
  const PostIdea({super.key, this.refreshFunc});

  @override
  State<PostIdea> createState() => _PostIdeaState();
}
...
class _PostIdeaState extends State<PostIdea> {
  ...
  Future<void> _getImage(ImageSource source) async {
    try {
      ...
      if (pickedFile != null) {
        ...
        if (_base64Image!.length > 500000) { // Reduced to 500KB to match backend limit
  ...
  Future<void> _handlePost() async {
    ...
    try {
      final success = await postIdea(controller.text, _base64Image, http.Client());
